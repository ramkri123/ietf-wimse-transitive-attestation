...
*   **Residency Verification (High Assurance)**: A high-assurance binding where the Workload Identity Agent itself is proven to be integral and rooted in hardware (e.g., TPM/TEE). This level of assurance is achieved by consuming the **Attestation Results** generated by the RATS-based **Verifiable Geofencing** framework.
...
: A nonce provided by the Resource Server or Workload Identity Agent specifically for the workload-to-agent fusion flow. It is distinct from the platform-level nonce (**N_platform**) used to verify the Workload Identity Agent itself.
